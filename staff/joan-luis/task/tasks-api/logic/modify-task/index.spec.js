...
const { env: { TEST_URL_TEST } } = process
const { expect } = require('chai')
const { ObjectId, database, models: { User, Task } } = require('tasks-data')
const modifyTask = require('.')
const { random } = Math
const { errors: { NotFoundError, ConflictError, ContentError }, polyfils: { arrayRandom } } = require('tasks-util')

arrayRandom()

describe('logic - modify task', () => {
    before(() => database.connect(TEST_URL_TEST))
    ...
    it('should fail on correct user and wrong task data', async() => {
        const {id}= await Task.findOne({ _id: { $nin: taskIds.map(taskId => ObjectId(taskId)) } })
